...
const { waitUntilBlock, wait } = require("./helpers/tempo")(web3);

contract("Farm", ([owner, alice, bob, carl, loftHolder]) => {
  beforeEach(async () => {
    ...
    this.farm.updateMultiplier(1, false);

    await this.erc20.transfer(loftHolder, 100000);
    await this.erc20.approve(this.farm.address, 100000, { from: loftHolder });
  });
  ...
  describe("before the start block", () => {
    it("allows participants to join", async () => {
      await this.lp.approve(this.farm.address, 1500, { from: alice });
      await this.lp.approve(this.farm.address, 500, { from: bob });
      await this.farm.deposit(0, 1500, { from: alice });
      await this.farm.deposit(0, 500, { from: bob });
      ...
      const balanceFarm2 = await this.lp.balanceOf(this.farm.address);
      assert.equal(2000, balanceFarm2);
    });
  });
  ...
  describe("with a 3th participant after 30 blocks", () => {
    it("reserved 2250 for alice, 750 for bob, and nothing for carl", async () => {
      ...
      await waitUntilBlock(10, this.startBlock + 30);
  ...
  describe("after 50 blocks of farming", () => {
    it("reserved 3000 for alice, 1000 for bob, and 1000 for carl", async () => {
      await this.lp.approve(this.farm.address, 1500, { from: alice });
      await this.lp.approve(this.farm.address, 500, { from: bob });
      await this.lp.approve(this.farm.address, 500, { from: carl });

      await this.farm.deposit(0, 1500, { from: alice });
      await this.farm.deposit(0, 500, { from: bob });
      await this.farm.deposit(0, 500, { from: carl });
      await waitUntilBlock(10, this.startBlock + 50);

      const pendingAlice = await this.farm.pending(0, alice);
      console.log(
        pendingAlice.toString(),
      ...
      await this.farm.updateMultiplier(10, true);

      const pendingAlice2 = await this.farm.pending(0, alice);
      console.log(
        pendingAlice2.toString(),
        await ethers.provider.getBlockNumber()
      );
      assert.equal(3060, pendingAlice2);

      const pendingBob2 = await this.farm.pending(0, bob);
      assert.equal(1020, pendingBob2);

      const pendingCarl2 = await this.farm.pending(0, carl);
      assert.equal(1020, pendingCarl2);

      await wait(10, 1);
      const pendingAlice3 = await this.farm.pending(0, alice);
      console.log(
        pendingAlice3.toString(),
        await ethers.provider.getBlockNumber()
      );
      assert.equal(3660, pendingAlice3);

      const pendingBob3 = await this.farm.pending(0, bob);
      assert.equal(1220, pendingBob3);

      const pendingCarl3 = await this.farm.pending(0, carl);
      assert.equal(1220, pendingCarl3);
    });
  });

  /*
  it("with a participant withdrawing after 10 blocks", async () => {
    await this.lp.approve(this.farm.address, 1500, { from: alice });
    await this.farm.deposit(0, 1500, { from: alice });
    ...
    const pendingAlice = await this.farm.pending(0, alice);
    console.log(pendingAlice.toString(),await ethers.provider.getBlockNumber());
    assert.equal(1000, pendingAlice);

    const balanceERC20 = await this.erc20.balanceOf(alice);
    assert.equal(0, balanceERC20);

    const balanceLP = await this.lp.balanceOf(alice);
    assert.equal(3500, balanceLP);

    //withdraw
    await this.farm.withdraw(0, 1500, { from: alice });
    const balanceERC202 = await this.erc20.balanceOf(alice);
    assert.equal(1000, balanceERC202);

    const balanceLP2 = await this.lp.balanceOf(alice);
    assert.equal(5000, balanceLP2);
  });
